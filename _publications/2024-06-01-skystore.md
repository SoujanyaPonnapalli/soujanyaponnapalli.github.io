...
keyinsight: 'Object stores that are designed for a single cloud leave managing data transfers across clouds and regions, guided by their applications'' access patterns, to users themselves. We redefine this as a caching problem, though a unique one: capacity is effectively unlimited, and the cost budget must account for moving data across regions and clouds. With SkyStore, we show that a TTL-based policy can replicate and evict data while minimizing cost and latency.'
