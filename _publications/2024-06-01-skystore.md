...
keyinsight: 'Object stores are designed for a single cloud, leaving users to manage data transfers across clouds and regions by hand, guided by their applications'' access patterns. We redefine this as a caching problem, though an unusual one: capacity is effectively unlimited, and the cost budget must account for moving data across regions and clouds. With SkyStore, we show that a TTL-based policy can replicate and evict data while minimizing cost and latency.'
